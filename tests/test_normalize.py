...
class TestNormalizer(unittest.TestCase):
    ...
    def test_crlf_conversion(self) -> None:
    ...
    def test_lf_conversion(self) -> None:
    ...
    def test_find_files(self) -> None:
        ...
        with open(os.path.join(subdir, "subfile.txt"), "w", encoding="utf-8") as f:
            f.write("Subdir file")
        with open(os.path.join(subdir, "code.py"), "w", encoding="utf-8") as f:
            f.write("print('hello')")
    ...
    def test_ignored_directories(self) -> None:
        ...
        # Custom ignore list
        custom_ignored = normalize.find_files(
            self.test_dir, [".txt"], ignore_dirs=["subdir"]
        )
        # Create a subdirectory with additional files if it doesn't exist
        subdir = os.path.join(self.test_dir, "subdir")
        if not os.path.exists(subdir):
            os.makedirs(subdir)
            with open(os.path.join(subdir, "ignored.txt"), "w", encoding="utf-8") as f:
    ...
    def test_non_utf8_files(self) -> None:
        ...
        # Instead of checking the exact bytes, let's just verify
    ...
    def test_parallel_processing(self) -> None:
        ...
        processed_count = normalize.process_files_parallel(
            test_files, "lf", False, True, max_workers=2  # Use 2 workers for testing
        )
    ...
    def test_file_pattern_handling(self) -> None:
        """Test handling of different file pattern formats."""
        # Create a separate test directory for this test to avoid
        # counting files from other tests
        ...
        for filename, content in test_files.items():
            with open(
                os.path.join(pattern_test_dir, filename), "w", encoding="utf-8"
            ) as f:
                f.write(content)
    ...
    def test_process_files_batch_processing(self) -> None:
    ...
    def test_cpu_count_none(self) -> None:
        """Test handling when CPU count is None."""
        files = [self.crlf_file]

        with patch("os.cpu_count", return_value=None):
            processed_count = normalize.process_files_parallel(files, "lf", False, True)
